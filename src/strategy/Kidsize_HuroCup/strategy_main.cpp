...
bool  WeightLifting::strategyBody(void)
{
	...
	switch (weightlifting_info->BodyState)
	{
		...
		case PickUp:
			...
			{	
				ros_com->sendBodyAuto(300,0,0,0,WalkingMode::ContinuousStep, SensorMode::None);
				tool->Delay(1500);
		...
		case tweak:
			...
			{
				ros_com->sendContinuousValue(800,weightlifting_info->continuous_Y,0,weightlifting_info->continuous_theta,SensorMode(weightlifting_info->continuous_imu));
			}
		...
		case checkmidline: //判斷中線距離
			...
			if((weightlifting_info->white[1][1]) > 160 && weightlifting_info->finallookline_flag)
			{
				...
				{
					weightlifting_info->second_speed = weightlifting_info->second_speed - 40;
					weightlifting_info->time_start = ros::WallTime::now().toSec()*1000;
				...
				{
					weightlifting_info->second_speed = weightlifting_info->second_speed - 150;
