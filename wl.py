    ...
    def main(self):
        if send.is_start:#啟動電源與擺頭
            ...
            if self.ctrl_status == 'preturn':
                ...
                if not self.body_auto:
                    ...
                    if send.DIOValue == 51:
                        ...
                        while self.bar.center.x >= 162 or self.bar.center.x <= 30 :
                            self.bar.update(1)
                            self.line.update(2)
            ...
            if self.ctrl_status == 'start_line':
                ...
                if send.imu_value_Yaw > 1.5 or send.imu_value_Yaw < -1.5:
                    if self.bar.center.x > 156:
                        send.sendContinuousValue(1000, -300, 0, -1, 0)
                        rospy.loginfo(f"右轉")
                    elif self.bar.center.x < 149 and self.bar.center.x > 0:
                        send.sendContinuousValue(1000, 0, 0, 1, 0)  
                        rospy.loginfo(f"左轉") 
                else :
                    if self.bar.center.x > 156:
                        ...
                        send.sendContinuousValue(1000, 100, 0, 0, 0)  
                        rospy.loginfo(f"左平移")  
                    else:
                        self.walking(1, 0)
                ...
                self.walking(0, -2)
                rospy.loginfo(f"white_YYY = {self.line.edge_min.y}")
